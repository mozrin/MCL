...
if (lexeme.empty() || lexeme[0] != '$') {
    ...
    if (it != keywords.end()) {
        if (it->second == TokenType::TRUE) return Token(TokenType::TRUE, lexeme, true, startLine);
        if (it->second == TokenType::FALSE) return Token(TokenType::FALSE, lexeme, false, startLine);
